...
LOGGER = get_logger("gcforest.data_cache")    # 拿日志
...
class DataCache(object):
    def __init__(self, config):
        ...
        if self.config.get("keep_in_mem") is None:
            self.config["keep_in_mem"] = {"default": 1}    # dict
        if self.config.get("cache_in_disk") is None:
            self.config["cache_in_disk"] = {"default": 0}  # 字典
        self.datas = {"train": {}, "test": {}}   # 训练集 测试集
    ...
    def get(self, phase, data_name, ignore_no_exist=False):
        ...
        """
        # 判断data_name是否为 str 或者 unicode 的实例
