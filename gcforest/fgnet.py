...
class FGNet(object):
    ...
    def fit_transform(self, X_train, y_train, X_test, y_test, train_config):
        ...
        self.data_cache.reset("train", X_train, y_train)    # 清空phase，重新加入
        if "test" in train_config.phases:
            self.data_cache.reset("test", X_test, y_test)
        for li, layer in enumerate(self.layers):   # 在每一层上训练
            layer.fit_transform(train_config)

    @staticmethod
    def concat_datas(datas):   # 拼接数据，行增加
        if type(datas) != list:
            return datas
